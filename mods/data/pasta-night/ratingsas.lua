...
function goodNoteHit(id, direction, noteType, isSustainNote)
	if not isSustainNote and resetHideHud and getProperty('combo') then
		...
		makeLuaSprite('lastRating', 'ratings/vanilla/' .. rating, screenWidth * 0.35 - -42 + downscrolloffset + 100 , screenHeight / 2 - 120 - downscrolloffset * 3 + 200 );
		...
		if combo >= 0 then
			for i=2,0,-1 do
				...
				makeLuaSprite(tag, 'ratings/vanilla/num' .. math.floor(combo / 10 ^ i % 10) , screenWidth * 0.35 - -105 - i * 43 + downscrolloffset + 100, screenHeight / 2 + -0 - downscrolloffset * 3 + 200 );
